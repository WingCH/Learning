	...
	@IBAction func togglePictureInPictureMode(_ sender: UIButton) {
		...
		*/
		if pictureInPictureController.isPictureInPictureActive {
            ...
            // Seek to 10 seconds before starting PiP
            let newTime = CMTimeMakeWithSeconds(10.0, 1)
            player.seek(to: newTime, toleranceBefore: kCMTimeZero, toleranceAfter: kCMTimeZero)
		...
		if AVPictureInPictureController.isPictureInPictureSupported() {
			...
			*/
            ...
            if #available(iOS 14.0, *) {
                // 這裡可以控制 PiP 的播放樣式： (tested in iPhone 17 Pro max iOS 26.1)
                // true: 線性播放模式 (Linear Playback)。隱藏進度條、快進/快退按鈕。適用於直播 (Live)。
                // false (預設): 標準點播模式 (VOD)。顯示進度條、快進/快退按鈕。
                // 注意：Apple 不允許自定義 PiP 視窗內的按鈕樣式或顏色。
